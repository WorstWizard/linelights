...
bool ray_aabb_intersect(Ray r, vec3 bbox_min, vec3 bbox_max) {
    ...
    double tx1 = (bbox_min.x - origin.x)*inv_d.x;
    double tx2 = (bbox_max.x - origin.x)*inv_d.x;

    double tmin = min(tx1, tx2);
    double tmax = max(tx1, tx2);

    double ty1 = (bbox_min.y - origin.y)*inv_d.y;
    double ty2 = (bbox_max.y - origin.y)*inv_d.y;

    tmin = max(tmin, min(ty1, ty2));
    tmax = min(tmax, max(ty1, ty2));

    double tz1 = (bbox_min.z - origin.z)*inv_d.z;
    double tz2 = (bbox_max.z - origin.z)*inv_d.z;
